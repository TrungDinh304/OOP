...
void CongTy::readfile(char* filename)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (so <= 15 && so >= 10)
		{
			list[i] = new NVSanXuat(manv, hoten, ngsinh, gioitinh, diachi, so);
			list[i]->tinhluong();
		}

		else if (so <= 26 && so >= 22)
		{
			list[i] = new NVCongNhat(manv, hoten, ngsinh, gioitinh, diachi, so);
			list[i]->tinhluong();
		}
	}
	fin.close();
...
void CongTy::xuat_file_luongthap()
{
	...
	ofstream fout;
	fout.open("emp_LowerAvgSalary.dat", ios::binary || ios::trunc);
	int count = 0;
	for (int i = 0; i < n; i++)
	{
		if (list[i]->tinhluong() < Luong_tr_binh())
			count++;
	}
	fout.write((char*)(&count), sizeof(count));
	cout << "Luong trung binh: " << (long)Luong_tr_binh() << endl;
	for (int i = 0; i < n; i++)
	{
		if (list[i]->tinhluong() < Luong_tr_binh())
		{
			NVCongNhat* congnhat = dynamic_cast<NVCongNhat*>(list[i]);
			NVSanXuat* sanxuat = dynamic_cast<NVSanXuat*>(list[i]);
			if (congnhat != NULL)
			{
				list[i]->write_binary_file(fout);
			}
			else
				list[i]->write_binary_file(fout);
		}
...
void CongTy::write_file_binary(char* filename)
{
	ofstream fout;
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (congnhat != NULL)
			...
			//fout.write((char*)sanxuat, sizeof(*sanxuat));
			list[i]->write_binary_file(fout);
	}
	fout.close();
}

void CongTy::read_file_binary(char* filename)
{
	ifstream fin;
	...
	for (int i = 0; i < n; i++)
	{
		...
		temp->read_binary_file(fin);
		if (temp->getSongay() <= 26 && temp->getSongay() >= 22)
		{
			list[i] = temp;
		}
		else
		{
			list[i] = new NVSanXuat(temp->getMa(), temp->getHoten(), 
				temp->getNgaysinh(), temp->getGioitinh(), temp->getDiachi(), temp->getSongay());
			delete temp;
		}
